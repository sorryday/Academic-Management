...
<head>
    ...
    <body>
        <div class="bg">
            ...
            $sql = "select * from user where name = '$name' and StudentID = '$sid' and ID = '$id'";

            $result = $conn->query($sql);
            if($result->num_rows > 0) {
                $row = $result->fetch_assoc(); 
            ?>
                <h3 class="result"><?=$name?>님의 비밀번호는 '<?=$row['PW']?>'입니다.</h3>
                <input type="button" onclick="location.href='login.html'" value="로그인 페이지">
            <?php
            }
            else { 
                print "<script language=javascript> alert('해당 계정이 없습니다.'); location.replace('findpw.html'); </script>";
             } ?>
            </div>
        </div>
    </body>
