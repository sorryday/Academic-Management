...
<html>
    ...
    <body>
        ...
        <form action="adShowAchieveProc3.php" method="post" name="upd">
        <div class="table" style="width: 80%; float: right;">
            ...
            <?php while($row = $result->fetch_assoc()) { ?>
                <input type="text" name="code[]" value="<?= $row['Code']?>" hidden>
            <tr>
                <td><?= $row['Code']?></td>
                <td><?= $row['Subject']?></td>
                <td><?= $row['Professor']?></td>
                <td><input type="text" value="<?= $row['stScore']?>" name="score[]"></td>
            </tr>
            <?php } ?>
            <tr>
                <td colspan="4"style="border:none;background-color: #FAFAFA; text-align:right;"><input type="submit" value="성적등록" class="btn"></td>    
            </tr>
            </table>
            <?php }
            else{ ?>
